// Inventory.jsx - FIXED
// Listens for "autobiz:data-changed" CustomEvent dispatched by Chat.jsx
// when Manager Agent performs any inventory or sales write.
// Calls fetchItems() immediately so the table reflects the change
...
const EMPTY = {
  name:'', sku:'', quantity:'', reorder_point:'', reorder_quantity:'',
...
export default function Inventory() {
  ...
  useEffect(() => {
    ...
    // Regular background poll every 10 seconds
    const poll = setInterval(fetchItems, 10000)

    // Immediate refresh triggered by Manager Agent writes via Chat.jsx
    const onDataChanged = e => {
      const type = e?.detail?.type || ''
      // inventory_write: direct inventory change
      // sales_write: may have deducted inventory stock
      if (type === 'inventory_write' || type === 'sales_write') {
        fetchItems()
      }
    }
    window.addEventListener('autobiz:data-changed', onDataChanged)

    return () => {
      clearInterval(poll)
      window.removeEventListener('autobiz:data-changed', onDataChanged)
    }
  }, [fetchItems])
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const msg = Array.isArray(d)
        ...
        : typeof d === 'string' ? d : 'An error occurred.'
  ...
  const onEdit = item => {
    ...
    setForm({
      ...
      holding_cost:       String(item.holding_cost       ?? ''),
      ordering_cost:      String(item.ordering_cost      ?? ''),
      supplier:           item.supplier           || '',
      supplier_lead_days: String(item.supplier_lead_days ?? ''),
    })
  ...
  const doDelete = async () => {
    ...
    try {
      ...
      addToast(`"${name}" removed from inventory.`, 'info')
      fetchItems()
